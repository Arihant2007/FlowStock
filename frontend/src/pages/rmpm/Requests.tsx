    ...
    <div className="space-y-4 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <ClipboardList className="h-6 w-6" /> Pending Approvals
        </h1>
        <p className="text-muted-foreground text-sm">Review and approve material requests.</p>
      </div>
      ...
      <Card>
        <CardContent className="pt-4 pb-0">
          ...
          >
            {requests.map((r) => (
              <Tr key={r.public_id} onClick={() => navigate(`/rmpm/requests/${r.public_id}`)}>
